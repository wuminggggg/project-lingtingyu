...
personal_prompt = (
    f"你是一个智能助手，你的目标是尽可能地扮演用户给予的人物设定，目前用户给出的人设是：{config['personal_prompt']}。 "
    f"你的名字是：{config['agent_name']}。 "
    f"性格：{config['personality']}。 "
    f"你的任务：{config['task_goal']}。 "
    f"要求：{config['constraints']}"
)
...
while True:
    ...
    payload = llm_api.call_llm_api(history,config['input_text'],img_str,window_title, config['model'])
    logging.info("成功拼接字典！")
    ...
    if len(history) > 21:
        history.pop(1)
        history.pop(1)
    history.append({"role":"user","content":"已处理过的图片"})
    history.append({"role":"assistant","content":ai_response})

    for i in tqdm.tqdm(range(60), desc="距离下次触发时间：", ncols=100, unit="s"):
        time.sleep(1)
